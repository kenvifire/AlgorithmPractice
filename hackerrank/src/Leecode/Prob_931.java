    ...
    public int minFallingPathSum(int[][] A) {
        ...
        for (int i = 1; i < A.length; i++) {
            for (int j = 0; j < A.length; j++) {
                ...
                if(j + 1 < A.length) min = Math.min(min, dp[i-1][j+1]);
                dp[i][j] = min + A[i][j];
            }
